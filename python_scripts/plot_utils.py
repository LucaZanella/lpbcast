...
def plot_view_distribution(file_name, num_of_processes):
    view_distribution = pd.read_csv("data/" + file_name)

    for i in range(0, num_of_processes):
        column_name = 'process_' + str(i)
        view_distribution[column_name] = view_distribution['view_distribution']\
    ...
    view_distribution.iloc[:, 2:].mean().plot(kind='bar')
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    num_of_processes = 18
    view_distribution_file_name = 'ModelOutput.2019.nov.18.15_49_02.txt'
    ...
    #plot_view_distribution(view_distribution_file_name, num_of_processes)
