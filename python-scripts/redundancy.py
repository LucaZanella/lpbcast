...
def plot_redundancy(opt_redundancies_file, not_opt_redundancies_file):
    opt_red_df = pd.read_csv("data/redundancy/" + opt_redundancies_file)
    not_opt_red_df = pd.read_csv("data/redundancy/" + not_opt_redundancies_file)
    ...
    g.savefig("figures/redundancy.png")
    plt.tight_layout()
    plt.show()
    plt.close()
